...
+ (instancetype)createChatViewToSuperView:(UIView *)superView {
    ...
    [QHChatViewUtil fullScreen:subView];
...
- (void)p_addTableView {
    ...
    [QHChatViewUtil fullScreen:tableV];
...
- (void)p_reloadAndRefresh:(BOOL)bRefreshImmediately {
    if (_bAutoReloadChat == YES) {
        if (_reloadTimer == nil || _reloadTimer.isValid == NO) {
            ...
            _reloadTimer = [NSTimer qhchateoc_scheduledTimerWithTimeInterval:_config.chatReloadDuration block:^{
